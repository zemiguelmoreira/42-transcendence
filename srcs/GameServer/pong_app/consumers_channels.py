...
class PongConsumer(AsyncWebsocketConsumer):
	...
	async def receive(self, text_data):
		...
		if self.is_player and 'action' in data:
			if data['action'] == 'move':
				...
				async with pong_game.locks[self.room.code]:
					self.rooms[self.room.code]['current_directions'][self.player_index] = direction


	async def send_game_updates(self):
		logger.info(f'Consumer: Send Game Updates Called user {self.user.username}\n')
		while not self.rooms['end_game'] and not self.rooms['disconnect']:
			async with pong_game.locks[self.room.code]:
				game_state = {
					'ball_position': self.rooms['ball_position'],
					'paddle_positions': self.rooms['paddle_positions'],
					'score': self.rooms['score'],
				}
	...
	async def start_game(self, event):
		...
		logger.info(f'-------------start game sent {self.user.username}\n')
		self.rooms = pong_game.rooms[self.room.code]
		logger.info(f'---------------before task {self.user.username}\n')
		self.task = asyncio.create_task(await self.send_game_updates())
		if self.player_index == 1 and len(self.rooms['players']) == 2:
